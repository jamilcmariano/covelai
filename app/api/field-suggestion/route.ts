...
export async function POST(request: Request) {
  try {
    ...
    const suggestion = await getFieldSuggestion(field, currentValue, jobTitle, companyName, request.headers)

    // Determine if this is a fallback response
    const isFallback = !process.env.GEMINI_API_KEY || suggestion.includes("Problem-solving, Communication, Teamwork")
    ...
    return NextResponse.json({ error: "Failed to get field suggestion" }, { status: 500 })
  }
}
