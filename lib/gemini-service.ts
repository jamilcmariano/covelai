...
export function generateFallbackLetter(
...
export async function generateCoverLetter(
  ...
  try {
    ...
    if (!process.env.GEMINI_API_KEY) {
      console.warn(
        "GEMINI_API_KEY not found, using fallback letter generation",
      );
      return generateFallbackLetter(request);
    }
    ...
    try {
      ...
      try {
        ...
        if (
          ...
          console.log(
            "Manually extracted evaluation:",
            manuallyExtractedEvaluation,
          );
...
export async function getFieldSuggestion(
  ...
  if (predefinedSuggestions[field]) {
    const jobSpecificSuggestion =
      predefinedSuggestions[field][jobTitle] ||
      predefinedSuggestions[field]["default"];
